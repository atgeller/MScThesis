...
While it would be possible to support them, we would have no more type information about them then \wasm, and the focus of this work is on the type information.
...
\begin{itemize}
    ...
    We do not reason about floating point values, so it is either a 32-bit integer ($i32$) or a 64-bit integer ($i64$).
...
The formal reason why certain \wasm instructions require run-time checks is because they have preconditions as part of their semantics, and if the preconditions are not met then those instructions trap to avoid undefined behavior (we've reproduced the reduction rules for those instructions in \autoref{fig:checked}).
...
We sometimes use the metavariable abbreviation $tfi ::= ti_1^{*};l_1;\phi_1 \rightarrow ti_2^{*};l_2;\phi_2$ as shorthand for the precondition and postcondition of an instruction.
...
It adds a new indexed type onto the stack to track the new program variable $\ti{t}{a}$, declares the new indexed type in the index type context $\phi$, and constrains that indexed type to be equal to the constant $(= a\; \ti{t}{c})$.
We require $a$ to be fresh, that is, we require that $a$ is a new index variable present nowhere else in any of the types for the program.
This is a common pattern to see in rules which introduce new index variables.
The local index store $l$ is unchanged between the precondition and postcondition.
\begin{mathpar}
...
Remember that indirect function calls perform a run time typecheck against the closure that they end up calling, so we assume statically that the check will proceed because if it does not the program will trap and not be able to do any harm.
...
The \refrule{Implies} says that if an indexed function type $tfi_1$ has a stronger precondition and weaker postcondition than some other indexed function type $tfi_2$, and is otherwise equivalent, then $tfi_1$ is a subtype of $tfi_2$ since it can safely be used in place of $tfi_2$.
...
\begin{mathpar}
    \inferrule*[right=\defrule{Call-Indirect-Prechk}]{ %% call_indirect
        C_{table}(i) = (n, (tfi_2 ...)) \\
        \phi \implies (\<gt>\; n\; a) \land (\<le> \ti{\<ithreetwo>}{0}\; a) \and
        tfis=(tfi_2 ...) \\
        tfi = ti_1^{*};l_1;\phi_1 \rightarrow ti_2^{*};l_2;\phi_2 \\
        \forall i.\; (\phi \implies \neg (= \ti{\<ithreetwo>}{i}\; a)) \lor\; tfis(i) <: tfi
    }
    {
        C \vdash \<callindirectpc> tfi :
        {\begin{stackTL}
...
Here we present a short contrived example of using types for check elimination.
We are typechecking a $\<divpc>$.
First, we give the module type context $C_1$, which contains one local variable, which is an $\<ithreetwo>$, and the instruction sequence we are typing, which is a safe division happening inside of a block (\autoref{fig:exampleprechk}).
...
The type of the block has an empty index type context $\circ$ because it requires no information to typecheck.
In the example we write the postcondition type of every instruction
in the line below the instruction (for space reasons), offset to the right.
Note that we omit type declarations in the index type context here for brevity.

Typingchecking the example relies on $\<select>$, where the second argument $(\<ithreetwo>.\<const> 1)$ is chosen if the first argument (and the conditional) $\<getlocal> 0$ is equal to zero.
Once we reach $\<select>$, the precondition index type context will be $(\ti{\<ithreetwo>}{a_1}\;\ti{\<ithreetwo>}{a_4}\;\ti{\<ithreetwo>}{a_5}\;{\<ithreetwo>}{a_6};\ti{\<ithreetwo>}{a_2});\circ,(= a_4\; a_6), (= a_5\; \ti{\<ithreetwo>}{1})$.
Then, after select, we have $(\ti{\<ithreetwo>}{a_1}\;\ti{\<ithreetwo>}{a_7};\ti{\<ithreetwo>}{a_2});\circ,(= a_4\; a_6), (= a_5\; \ti{\<ithreetwo>}{1}), (\text{if }(= a_6\; \ti{\<ithreetwo>}{0})\;(= a_7 a_5) (= a_7 a_4))$.
We know $(= a_4 a_6)$ since $(= a_4 a_2)$ and $(= a_6 a_2)$.
Since $(= a_4\; a_6)$, if $(= a_6\; \ti{\<ithreetwo>}{0})$ we choose $a_5$ which is 1, else we choose $a_4$, which must not be 0.
Thus, $\circ,(= a_4\; a_6), (= a_5\; \ti{\<ithreetwo>}{1}), (\text{if }(= a_6\; \ti{\<ithreetwo>}{0})\;(= a_7\; a_5) (= a_7\;a_4)) \implies \neg(= a_7\; \ti{\<ithreetwo>}{0})$.
...
\begin{figure}
    ...
    $e^{*} = \<block>\;
    {\begin{stackTL}
        (\ti{\<ithreetwo>}{a_1}\;\ti{\<ithreetwo>}{a_2};\ti{\<ithreetwo>}{a_3};\circ \rightarrow \ti{\<ithreetwo>}{a_0};\ti{\<ithreetwo>}{a_2};\circ)
        \\ (\<teelocal> 0)
        \\ \hfill {\begin{stackTL}
            \ti{\<ithreetwo>}{a_0}\;\ti{\<ithreetwo>}{a_4};
            \\\ti{\<ithreetwo>}{a_2};
            \\(= a_4\;a_2)
        \end{stackTL}}
        \\ (\<ithreetwo>.\<const> 1)
        \\ \hfill \ti{\<ithreetwo>}{a_0}\;\ti{\<ithreetwo>}{a_4}\;\ti{\<ithreetwo>}{a_5};\ti{\<ithreetwo>}{a_2};(= a_4\;a_2),(= a_5\;\ti{\<ithreetwo>}{1})
        \\ (\<getlocal> 0)
        \\ \hfill {\begin{stackTL}
            \ti{\<ithreetwo>}{a_0}\;\ti{\<ithreetwo>}{a_4}\;\ti{\<ithreetwo>}{a_5}\;\ti{\<ithreetwo>}{a_6};
            \\ \ti{\<ithreetwo>}{a_2};
            \\ (= a_4\;a_2),(= a_5\;\ti{\<ithreetwo>}{1}),(= a_6\; a_2)
            \end{stackTL}}
        \\ (\<select>)
        \\ \hfill {\begin{stackTL}
            (\ti{\<ithreetwo>}{a_1}\;\ti{\<ithreetwo>}{a_7};
            \\\ti{\<ithreetwo>}{a_2});
            \\ (= a_4\; a_6), (= a_5\; \ti{\<ithreetwo>}{1}),
            \\ (\text{if }(= a_6\; \ti{\<ithreetwo>}{0})\;(= a_7\;a_5) (= a_7\;a_4))
        \end{stackTL}}
        \\ (\<ithreetwo>.\<divpc>)
    \end{stackTL}} \\
    \<nsend>$

    \caption{\name Example of Typechecking a Prechk Instruction}
    \label{fig:exampleprechk}
\end{figure}
