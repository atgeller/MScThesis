...
We require $a$ to be fresh, which is enforced by the premise $a \not\in \phi$, this is a common pattern to see in rules which introduce new index variables.
...
\begin{mathpar}
    ...
    \inferrule*[right=\defrule{Testop}]{a_2 \not\in \phi}{ %% testop
...
\begin{mathpar}
    ...
    \inferrule*[right=\defrule{Set-Local}]{ %% set_local
        C_{\text{local}}(i) = t \and
        l_2 = l_1 \text{ except } l_2(i) = \ti{t}{a_2} \and
        a_2 \not\in \phi
    }
    {
...
\todo{Properly format digression}
\paragraph{Digression on Fresh Variables}
\todo{Is this clunky? This feels clunky, but I also think that the example helps and connects it to what we were saying above.}
...
We can now add fresh index variables and arbitrary constraints on them to the postconditions of indexed function types as a part of weakening.
This may seem a little counterintuitive since it looks like we are strengthening the constraint set, but it is in fact unchanged with respect to the index variables that are actually used in the program).
Further, we are careful to avoid contradictions by requiring that the new index variables are fresh.
In the example we just walked through, we could safely add $b$ because it was not a variable we cared about and was not present in $\circ,\ti{\<ithreetwo>}{a},(> a \ti{\<ithreetwo>}{10})$, so we knew it would not cause any contradictions.
