...
\begin{figure}[h]
    \ContinuedFloat
    \begin{mathpar}
        \inferrule[]{ %% state entailment
            s_\text{glob}(i) = (t_g.\<const> c_g)^{*}\\
            v^{*} = (t_l.\<const> c_l)^{*}\\
            \phi_s = \circ,\ti{t_l}{a_l}^{*},(\<eq> a_l \; \ti{t_l}{c_l})^{*},\ti{t_g}{a_g}^{*},(\<eq> a_g \; \ti{t_g}{c_g})^{*} \\
            \phi_s \implies \phi
        } {
            s;v^{*} \vdash_i \ti{t_l}{a_l}^{*};\ti{t_g}{a_g};\phi
        } \and
        \inferrule{ %% admin program
            \vdash s : S \and
            S;\epsilon \vdash_i s;v^{*};e^{*} : ti^{*};l;g;\phi
        } {
            \vdash_i s;v^{*};e^{*} : ti^{*};l;g;\phi
        } \and
        \inferrule{ %% admin code
            s;v^{*} \vdash_i l_1;g_1;\phi_1\\
            C = S_{\text{inst}}(i),\text{local} \; t_v^{*}, \text{return} \; (ti^n;l_2;g_2;\phi_2)^{?}\\
            S;C \vdash_i e^{*} : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_2;g_2;\phi_2
        } {
            S;(ti^n;l_2;g_2;\phi_2)^{?} \vdash_i s;v^{*};e^{*} : ti^n;l_2;g_2;\phi_2
        } \and
        \inferrule[]{ %% admin const
        } {
            \vdash t.\<const> c : \ti{t}{a};\ti{t}{a},(\<eq> a \; \ti{t}{c})
        } \and
        \inferrule[]{ %% trap
        } {
            C \vdash \<trap> : tfi
        ...
        \inferrule[]{ %% local same instance
            S;(ti^n;l_2;g_2;\phi_2) \vdash_j s;v_l^{*};e^{*} : ti^n;l_2;g_2;\phi_2
        } {
            S;C \vdash_j s;v^{*};\<local>_n \{ j;v_l^{*} \} \; e^{*} \<end> : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2
        } \and
        \inferrule[]{ %% local different instance
            S;(ti^n;l_2;g_4;\phi_2) \vdash_i s;v_l^{*};e^{*} : ti^n;l_2;g_4;\phi_2\\
            S_\text{global}(j) = (\text{mut}^{?} \; t)^{*} \and
            i \neq j
        } {
            S;C \vdash_j s;v^{*};\<local>_n \{ i;v_l^{*} \} \; e^{*} \<end> : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;\ti{t}{a}^{*};\phi_2
        } \and
    \end{mathpar}
