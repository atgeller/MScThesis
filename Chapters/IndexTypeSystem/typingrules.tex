...
\begin{figure}[h]
    \begin{mathpar}
        ...
        \inferrule[]{ %% return
            C_{\text{return}}(i) = ti^{*};l;g_1;\phi \and
            \phi_1 \implies \phi
        }
        {
...
\begin{figure}[h]
    \ContinuedFloat
    \begin{mathpar}
        \inferrule{ %% run time program typing
            \vdash s : S \and
            (\vdash s_{\text{glob}}(i) : \ti{t}{a};\epsilon;\epsilon;\phi_g)^{*} \and\\
            S;\epsilon \vdash_i v^{*};e^{*} : \epsilon;\epsilon;\ti{t}{a}^{*};\phi_g^{*} \rightarrow ti^{*};l_2;g_2;\phi_2
        } {
            \vdash_i s;v^{*};e^{*} : ti^{*};l_2;g_2;\phi_2
        } \and
        \inferrule{ %% ??? v1
            C = S_{\text{inst}}(i),\text{local} \; t_v^{*}, \text{return} \; (ti^n;l_2;g_2;\phi_2)^{?} \and\\
            S;C \vdash e^{*} : \epsilon;\ti{t_v}{a}^{*};g_1;\phi_1,\ti{t_v}{a}^{*},(\<eq> a \; \ti{t_v}{c})^{*} \rightarrow ti^n;l_2;g_2;\phi_2
        } {
            S;(ti^n;l_2;g_2;\phi_2)^{?} \vdash_i (t_v.\<const> c)^{*};e^{*} : \epsilon;\epsilon;g_1;\phi_1 \rightarrow ti^n;l_2;g_2;\phi_2
        } \and
        \inferrule{ %% ??? v2
            C = S_{\text{inst}}(i),\text{local} \; t_v^{*}, \text{return} \; (ti^n;l_2;g_2;\phi_2)^{?} \and\\
            (\vdash v : \ti{t_v}{a};\epsilon;\epsilon;\phi_v)^{*} \and\\
            S;C \vdash e^{*} : \epsilon;\ti{t_v}{a}^{*};g_1;\phi_1,\phi_v^{*} \rightarrow ti^n;l_2;g_2;\phi_2
        } {
            S;(ti^n;l_2;g_2;\phi_2)^{?} \vdash_i v^{*};e^{*} : \epsilon;\epsilon;g_1;\phi_1 \rightarrow ti^n;l_2;g_2;\phi_2
        } \and
        \inferrule[]{ %% admin const
        } {
            \vdash t.\<const> c : \ti{t}{a};\epsilon;\epsilon;\ti{t}{a},(\<eq> a \; \ti{t}{c})
        } \and
        \inferrule[]{ %% trap
        } {
            C \vdash \<trap> : tfi
        } \and
        \inferrule[]{ %% local same instance
            S;(ti^n;l_2;g_2;\phi_2) \vdash_i v_l^{*};e^{*} : \epsilon;\epsilon;g_1;\phi_1 \rightarrow ti^n;l_2;g_2;\phi_2
        } {
            S;C \vdash_i \<local>_n \{ i;v_l^{*} \} \; e^{*} \<end> : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2
        } \and
        \inferrule[]{ %% local different instance
            S;(ti^n;l_2;g_4;\phi_2) \vdash_i v_l^{*};e^{*} : \epsilon;\epsilon;g_3;\phi_1 \rightarrow ti^n;l_2;g_4;\phi_2 \and
            i \neq j
        } {
            S;C \vdash_j \<local>_n \{ i;v_l^{*} \} \; e^{*} \<end> : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2
        } \and
    \end{mathpar}
