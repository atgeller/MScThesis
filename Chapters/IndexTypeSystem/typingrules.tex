...
\begin{figure}[h]
    ...
    \begin{mathpar}
        ...
        \inferrule[]{ %% fresh globals
            C \vdash e^{*} : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_2;g_2;\phi_2
            C_\text{globals} = (\text{mut}^{?} \; t)^{*}
        }
        {
            C \vdash e^{*} : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_2;\ti{t}{a}^{*};\phi_2,\ti{t}{a}^{*}
        } \and
        \inferrule[]{ %% combine
...
\begin{figure}[h]
    ...
    \begin{mathpar}
        ...
        \inferrule[]{ %% local same instance
            S;(ti^n;l_2;g_2;\phi_2) \vdash_j v_l^{*};e^{*} : \epsilon;\epsilon;g_1;\phi_1 \rightarrow ti^n;l_2;g_2;\phi_2
        } {
            S;C \vdash_j \<local>_n \{ j;v_l^{*} \} \; e^{*} \<end> : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2
        } \and
        \inferrule[]{ %% local different instance
            ...
            S_\text{global}(j) = (\text{mut}^{?} \; t)^{*} \and
            i \neq j
        } {
            S;C \vdash_j \<local>_n \{ i;v_l^{*} \} \; e^{*} \<end> : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;\ti{t}{a}^{*};\phi_2
        } \and
    \end{mathpar}
