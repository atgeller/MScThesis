...
\begin{itemize}
    ...
    \item Case: $\<local>_n \{ i;v^{*}_l \} \; L^k[v^n \; \<return>] \<end> \hookrightarrow_j \; v^n$
        ...
        \begin{itemize}
            ...
            $S;(ti^n;l_2;g_2;\phi_2) \vdash_j v_l^{*};L^k[v^n \; \<return>] : \epsilon;\epsilon;g_1;\phi_1 \rightarrow ti^n;l_2;g_4;\phi_2$ because it is a premise of $local-same-inst$ which we have assumed to hold.

            $(\vdash v_l : ti_l;\epsilon;\epsilon;\phi_v)^{*}$\\ $\land$
            $S;C_l \vdash_j L^k[v^n \; \<return>] : \epsilon;ti_l^{*};g_1;\phi_1,\phi_v^{*} \rightarrow ti_n;l_2;g_4;\phi_2$,
            ...
            \thought{We know such a $\phi_3$ exists because the expression is well-typed?}

            $S;C_l \vdash_j v^n \; \<return> : s_3;l_2;g_2;\phi_5 \rightarrow s_3 \; ti^n;l_2;g_2;\phi_3$ by $composition$.

            $S;C_l \vdash_j v^n : \epsilon;l_2;g_2;\phi_3 \rightarrow ti^n;l_2;g_2;\phi_2$ by $const$.

            $S;C_l \vdash_j v^n : \epsilon;l_2;g_2;\phi_3 \rightarrow ti^n;l_2;g_2;\phi_2$ by $subtyping$.

            $S;C \vdash_j v^n : \epsilon;l_1;g_1;\phi_3 \rightarrow ti^n;l_2;g_2;\phi_2$ by $const$.
