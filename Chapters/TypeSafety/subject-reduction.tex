...
\begin{itemize}
    ...
    % \item Case: $S;C \vdash_i \<local>_n \{ i;v^{*}_l \} \; v^n \<end> : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_2;g_2;\phi_2$
    % \\ $\land$ $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_i \; v^n$
    ...
    \item Case: $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_j \; v^n$
        ...
        \begin{itemize}
            \item Case: $i = j$
            \\ $\land$ $S;C \vdash_i \<local>_n \{ i;v^{*}_l \} \; v^n \<end> : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_1;g_2;\phi_2$

                By $inversion$, $s_2=s_1\;ti^n$.

                $S;(t^n) \vdash_i v^{*};v^n : ti^n;l_3;g_2;\phi_2$ because it is a premise of $local-same-inst$ which we have assumed to hold.

                $S;C \vdash_i \<local>_n \{ i;v^{*}_l \} \; v^n \<end> : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_2;g_2;\phi_2$

            \item Case: $i \neq j$
            \\ $\land$ $S;C \vdash_j \<local>_n \{ i;v^{*}_l \} \; v^n \<end> : s_1;l_1;g_1\phi_1 \rightarrow s_2;l_1;g_2;\phi_2$

        \end{itemize}

    % \item Case: $S;C \vdash \<local>_n \{ i;v^{*}_l \} \; v^n \<end> : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_2;g_2;\phi_2$
    % \\ $\land$ $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_j \; v^n$

    %     $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_2;g_2;\phi_2$ by $local$ and $inversion$.

    %     By $inversion$, we know $s_2=s_1\;ti^n$.

    %     $S;(ti^n) \vdash_i v_l^{*};v^n : ti^n;ti_l^{*};g_2;\phi_2$ from the premise of $local$ which we have assumed to hold.

    %     \todo{Create a typing rule for \<local> so that it can be invoked with inversion in this case (this requires store typing rules).}

    %     \todo{Prove that since $v^n$ cannot depend on the value of locals, $S;\epsilon \vdash_i v^n : ti^n;l_2;g_2;\phi_2$}

    %     \todo{What rule do we invoke to replace $\epsilon$ with $C$? $weakening$?}

    %     Therefore, $S;C \vdash v^n : s_1;l_1;g_1;\phi_1 \rightarrow s_1\;ti^n;l_1;g_1;\phi_2$ by $stack-poly$.

    %     \thought{$\phi_2$ maybe should be $\phi_3$ where $\phi_3 \implies \phi_2$ and contains no references to local values in $l_2$ not in $l_1$.}

    %     \thought{Might also need to use the fact that local blocks can only reference local values inside their local block}
    ...
    % \item Case: $S;C \vdash \<local>_n \{ i;v_l^{*} \} \; L^k [v^n \; \<return>] \<end> : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_2;g_2;\phi_2$
    % \\ $\land$ $\<local>_n \{ i;v_l^{*} \} \; L^k [v^n \; \<return>] \<end> \hookrightarrow \; v^n$

    %     By $inversion$, we know $s_2=s_1\;ti^n$.

    %     $S;(ti^n) \vdash_i v_l^{*}; L^k [v^n \; \<return>] \<end> : ti^n;l_2 \; v_l^{*};g_2;\phi_2$ from the premise of $local$ which we have assumed to hold.
