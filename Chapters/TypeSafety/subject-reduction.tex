    ...
    \item Case: $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_j \; v^n$
        ...
        \begin{itemize}
            ...
            \\ $\land$ $S;C \vdash_i \<local>_n \{ i;v_l^{*} \} \; v^n \<end> : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_1;g_2;\phi_2$
                ...
                $C \vdash v^n : \epsilon;\ti{t}{a}^{*};g_1;\phi_1,\ti{t}{a}^{*},(\<eq> a\; \ti{t}{c})^{*} \rightarrow ti^n;l_2;g_2;\phi_2$ because values have the same type under any context.
                \\\thought{lemma?}

                Because $a^{*}$ are fresh, $\phi_1 \implies \phi_1,\ti{t}{a}^{*},(\<eq> a\; \ti{t}{c})^{*}$.
                \\\thought{might not be able to appeal just to freshness? could need justification that the fresh variables are never referenced by values.}

                $C \vdash v^n : \epsilon;\ti{t}{a}^{*};g_1;\phi_1 \rightarrow ti^n;l_2;g_2;\phi_2$ by $weakening$.

                $l_2 = \ti{t}{a}^{*}$ by $const$ and $inversion$.
                \\\todo{Is this needed?}

                $C \vdash v^n : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2$ by $const$.
                \\\thought{lemma that the locals can be swapped since $const$ only requires pre and post condition locals are the same?}
            ...
            \\ $\land$ $S;C \vdash_j \<local>_n \{ i;v_l^{*} \} \; v^n \<end> : s_1;l_1;g_1\phi_1 \rightarrow s_2;l_1;g_2;\phi_2$
                ...
                \thought{$g_3$ and $g_4$ here refer to globals in instance $i$, which is kinda weird?}
                ...
                $C \vdash_i v^n : \epsilon;\ti{t}{a}^{*};g_3;\phi_1,\ti{t}{a}^{*},(\<eq> a\; \ti{t}{c})^{*} \rightarrow ti^n;l_4;g_4;\phi_2$ because values have the same type under any context.

                Because $a^{*}$ are fresh, $\phi_1 \implies \phi_1,\ti{t}{a}^{*},(\<eq> a\; \ti{t}{c})^{*}$.

                $C \vdash_i v^n : \epsilon;\ti{t}{a}^{*};g_3;\phi_1 \rightarrow ti^n;l_4;g_4;\phi_2$ by $weakening$.

                $\ti{t}{a}^{*} = l_4$, and $g_3 = g_4$ by $const$ and $inversion$.

                $C \vdash_i v^n : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2$ by $const$.
                \\\thought{same lemma as in $i = j$ case, but also changing globals}
                \\\thought{this is really weird, since we swap out the global stores, but are still typing in instance $i$? Like if we can do that what does typing under a given instance number even mean?}

                Therefore, $C \vdash_j v^n : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2$ by $???$.
                \\\thought{this shows something very weird about using the instance number in the typing, like what kind of claim would this even be?}
