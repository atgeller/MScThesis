...
\begin{itemize}
    ...
    \\ $\land$ $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_j \; v^n$
        ...
        \thought{this assumes it's legal to remove from the annotations}

        Because $a^{*}$ are free in $\phi_v$ and $ti^n$, $\satisfies{\phi_v}{ti^n}{\phi_l^{*},\phi_v}$.
        ...
        \thought{stuck here, not sure how to justify using the satisfaction with $ti^n$, would require adding to the annotations, which seems iffy}.
    ...
    \\ $\land$ $v_1^j \; v \; v_2^k;v' \; \<setlocal> j \hookrightarrow v_1^j \; v' \; v_2^k\epsilon$
        ...
        By $inversion$ on $admin-const$, $t.\<const> c = v$, and $\phi_v = \circ,\ti{t}{a},(= a \; \ti{t}{c})$.
        ...
        $t.\<const> c' = v'$, $ti^{*} = \epsilon$, $l_1 = l_3$,
        and $\satisfies{\phi_1^j,\phi_v,\phi_2^k,\ti{t}{a'},(= a' \; \ti{t}{c'})}{\ti{t}{a'}\;l_1}{\phi_3}$.
        ...
        \thought{potential unsoundness below.}

        $\vdash v' : \ti{t}{a'};\circ,\ti{t}{a'},(= a' \; \ti{t}{c})$

        Since $a$ is free in $l$, $\ti{t}{a'}$, and $\phi_1^j,\phi_2^k,\ti{t}{a'},(= a' \; \ti{t}{c')$,
        $\satisfies{\phi_1^j,\phi_2^k,\ti{t}{a'},(= a' \; \ti{t}{c'})}{l}{\phi_3}{\phi_1^j,\phi_v,\phi_2^k,\ti{t}{a'},(= a' \; \ti{t}{c'})}$.

        \todo{...}

        $C \vdash \epsilon : \epsilon;l;\phi_1^j,\ti{t}{a_l},(= a_l\;\ti{t}{c'}),\phi_2^k \rightarrow \epsilon;l;\phi$
