...
\begin{itemize}
    ...
    \\ $\land$ $v_1^j \; v \; v_2^k;v' \; \<setlocal> j \hookrightarrow v_1^j \; v' \; v_2^k\epsilon$
        ...
        % \thought{If $ti_1^{*};l_1;\phi_0 \rightarrow ti_2^{*};l_2;\phi_3 <: ti_1^{*};l_1;\phi_1 \rightarrow ti_2^{*};l_2;\phi_2$}

        \thought{
            Want to show $S;\epsilon \vdash_i v_1^j \; v' \; v_2^k; \epsilon : ti^{*};l;\phi$.
            Want to show $C \vdash \epsilon : \epsilon;l;\phi_1^j,\phi_{v'},\phi_2^k \rightarrow ti^{*};l;\phi$.
            Have $ti^{*} = \epsilon$, so need to show that $\epsilon;l;\phi_1^j,\phi_{v'},\phi_2^k \rightarrow \epsilon;l;\phi_1^j,\phi_{v'},\phi_2^k <: \epsilon;l;\phi_1^j,\phi_{v'},\phi_2^k \rightarrow \epsilon;l;\phi$.
            Can't get that $\phi_1^j,\phi_{v'},\phi_2^k \implies \phi$ since can't invert $subtyping$.
        }
