    ...
    \item Case: $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_j \; v^n$
        ...
        \begin{itemize}
            ...
            \\ $\land$ $S;C \vdash_i \<local>_n \{ i;v_l^{*} \} \; v^n \<end> : s_1;l_1;g_1;\phi_1 \rightarrow s_2;l_1;g_2;\phi_2$
                ...
                $S;(t^n) \vdash_i v_l^{*};v^n : \epsilon;l_3;g_1;\phi_1 \rightarrow ti^n;l_4;g_2;\phi_2$ because it is a premise of $local-same-inst$ which we have assumed to hold.

                $S;S_{inst}(i),\text{local} t_v^{*}, \text{return} (t^n) \vdash v^n : \epsilon;l_3;g_1;\phi_3 \rightarrow ti^n;l_4;g_2;\phi_2$, because it is a premise of $with-return$ which we have assumed to hold, and $\phi_3 \implies \phi_1$.

                $C \vdash v^n : \epsilon;l_3;g_1;\phi_1 \rightarrow ti^n;l_4;g_2;\phi_2$ because values have the same type under any context.

                $C \vdash v^n : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2$ becuase \\\todo{justification}.

                \thought{Well, the rules for consts assumes nothing about the pre-/post-conditions on the locals and globals, as long as they are the same.}

                \thought{To do it a little more formal, you appeal to inversion to get from $v^n$ to the exact post-condition ($\phi_1$ extended with the consts, which you also know implies $\phi_2$), then you can just use the const rule to derive the second part.}

                \thought{Maybe we could just use the rule for typing consts to show that if the expression is well-typed, $g_1 = g_2$, and $\phi_1 = \phi_2$?}
            ...
            \\ $\land$ $S;C \vdash_j \<local>_n \{ i;v_l^{*} \} \; v^n \<end> : s_1;l_1;g_1\phi_1 \rightarrow s_2;l_1;g_2;\phi_2$
                ...
                $S;(t^n) \vdash_j v_l^{*};v^n : \epsilon;l_3;g_3;\phi_1 \rightarrow ti^n;l_4;g_4;\phi_2$ because it is a premise of $local-diff-inst$ which we have assumed to hold.

                $S;S_{\text{inst}}(i),\text{local} t_v^{*}, \text{return} (t^n) \vdash v^n : \epsilon;l_3;g_3;\phi_3 \rightarrow ti^n;l_4;g_4;\phi_2$, because it is a premise of $with-return$ which we have assumed to hold, and $\phi_3 \implies \phi_1$.

                $C \vdash v^n : \epsilon;l_3;g_3;\phi_3 \rightarrow ti^n;l_4;g_4;\phi_2$ because values have the same type under any context.

                $C \vdash v^n : \epsilon;l_1;g_1;\phi_3 \rightarrow ti^n;l_1;g_2;\phi_2$ because \\\todo{justification}.

                \thought{Note, the same principle as above could be applied to showing that $l_3 = l_4$ and $g_3 = g_4$, but this isn't helpful.}

                $C \vdash v^n : \epsilon;l_1;g_1;\phi_1 \rightarrow ti^n;l_1;g_2;\phi_2$ by $weakening$? \\\todo{This doesn't work, can replace $\phi_3$ with $\phi_1$ if $\phi_1 \implies \phi_3$, we have $\phi_3 \implies \phi_1$}
