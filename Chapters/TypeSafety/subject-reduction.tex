...
\begin{itemize}
    ...
    \\ $\land$ $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_j \; v^n$
        ...
        $S;(ti^n;l_3;\phi_3) \vdash_i s;v_l^{*};v^n : ti^n;l_3;\phi_3$ because it is a premise of $local$ which we have assumed to hold.
    ...
    \\ $\land$ $\<local>_n \{ i;v_l^{*} \} \; L^k[v^n \; \<return>] \<end> \hookrightarrow_j \; v^n$
        ...
        $S;(ti^n;l_3;\phi_3) \vdash_i s;v_l^{*};L^k[v^n \; \<return>] : ti^n;l_3;\phi_3$ because it is a premise of $local$ which we have assumed to hold.

        $(\vdash v_l : ti_l;\phi_l)^{*}$, and\\
        $S;C_l \vdash s;v_l^{*};L^k[v^n \; \<return>] : \epsilon;ti_l^{*};\phi_l^{*} \rightarrow ti^n;l_3;\phi_3$, where\\
        ...
        $S;C_l \vdash s;v_l^{*};\<return> : ti_3^{*} \; ti^n;l_3;\phi_3 \rightarrow ti_4^{*};l_4;\phi_4$ by $return$.

        $S;C_l \vdash s;v_l^{*};v^n : \epsilon;l_3;\phi_5 \rightarrow ti^n;l_3;\phi_6$, where $\phi_6 \implies \phi_3$ by $inversion$ on $const$, $composition$, and $subtyping$.

        $S;C_l \vdash s;v_l^{*};v^n : \epsilon;l_3;\phi_l^{*} \rightarrow ti^n;l_3;\phi_6$ by (Nested-Type-Preserved).
