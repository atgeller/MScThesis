...
\begin{itemize}
    ...
    \\ $\land$ $s;v \; (\<setglobal> j) \hookrightarrow_i s';\epsilon$, where $s' = s$ with $\text{glob}(i,j) = v$
        ...
        Because $a$ is fresh in $ti_2^{*}$, $l_2$, and $\phi_1$, $\phi_1 \implies \phi_1,\ti{t}{a},(= \; \ti{t}{c})$.
        ...
        $S \vdash s'_\text{inst}(i) : C$ by $instance$ and the premises of $S \vdash s_\text{inst}(i) : C$.

        \thought{Maybe I should explicitly invoke the other premises?}

        Therefore, $\vdash s' : S$ by $store$ and the premises of $\vdash s : S$.
