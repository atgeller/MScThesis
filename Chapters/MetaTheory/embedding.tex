...
\thought{
    The sound instruction typing embedding lemma is saying that there exists a type of the required form.
    We can choose the exact values, such as $l_1 = l_2$, because that still fits the form.
    The function proof just requires that this lemma proves an existential for a type of the given form (with the exact raw types) since it is only making existential claims on the exact index types.
    But the inductive hypothesis in the composition case requires that either the stack precondition or the post condition are treated as fixed, otherwise that proof case would have values that are choosen at two independent points.
    I think it would be easiest to treat the precondition as fixed.
}
