...
\begin{lemma}{Sound Instruction Typing Embedding}
    ...
    \}$,
    % note that the label types include the locals types, but that the return type doesn't
    % make sure the function definitions match
    $(\erase[\tfi]{\tfi} = \tf)^{*}$,
    \\ then $C_p \vdash (\embed[e]{e}^{(t_l^{*})})^{*} : ti_1^{*};l_1;\phi_1 \rightarrow ti_2^{*};l_2;\phi_2$, where
    % these are to make the definitions not quite as long, might be overcomplicating things
    $\phi_1 = \circ,l_1,ti_1^{*}$.
    $l_1 = \ti{t_l}{a_{l1}}$,
    $ti_1^{*} = \ti{t_1}{a_1}^{*}$,
    $ti_2^{*} = \ti{t_2}{a_2}^{*}$,
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Therefore, $C_p \vdash \epsilon : \epsilon;l_1;\phi_1 \rightarrow \epsilon;l_2;\phi_2$ by \refrule{Empty}.
        ...
        \thought{I'm not sure if these let statements require more justification...}
        ...
        $C_p \vdash (\embed[e]{e}^{(t_l^{*})})^{*} : \ti{t_3}{a_3}^{*};l_1;\phi_1 \rightarrow \ti{t_4}{a_4}^{*};l_2;\phi_2$ by \refrule{Implies}.

        $C_p \vdash (\embed[e]{e}^{(t_l^{*})})^{*} : ti_1^{*};l_1;\phi_1 \rightarrow ti_2^{*};l_2;\phi_2$ by \refrule{Stack-Poly}.
