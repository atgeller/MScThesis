...
\begin{itemize}
    ...
    \\ $\land$ $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_j \; v^n$
        ...
        By \reflemma{Inversion} on \refrule{Code}, $(\vdash v_l : ti_l;\phi_l)^{*}$,
        and $S;C_l \vdash v^n : \epsilon:ti_l^{*};\phi_l^{*} \rightarrow ti^n;l_3;\phi_3$.

        By \reflemma{Inversion} on \refrule{Admin-Const}, $\phi_l^{*} = \circ,\ti{t}{a}^{*},(= a \ti{t}{c})^{*}$.
    ...
    \\ $\land$ $\<local>_n \{ i;v_l^{*} \} \; L^k[v^n \; \<return>] \<end> \hookrightarrow_j \; v^n$
        ...
        By \reflemma{Inversion} on \refrule{Admin-Const}, $ti_l^{*} = \ti{t_l}{a_l}^{*}$.
        ...
        Since $a_l^{*}$ are fresh, $\circ \implies \phi_l^{*}$.
    ...
    \\ $\land$ $v_1^j\;v\;v_2^k;\<getlocal> j \hookrightarrow v$
        ...
        By \reflemma{Inversion} on \refrule{Code},
        $\vdash v : \ti{t}{a};\phi_v$,
        and $S;C \vdash \<getlocal> j : \epsilon;l_1;\phi_1^j,\phi_v,\phi_2^k \rightarrow ti^{*};l;\phi$.

        By \reflemma{Inversion} on \refrule{Get-Local},
        $ti^{*} = \ti{t}{a}$, $l_1 = l$, and $\phi_1^j,\phi_v,\phi_2^k \implies \phi$.

        By \reflemma{Inversion} on \refrule{Admin-Const},
        $t.\<const> c = v$, and $\phi_v = \circ,\ti{t}{a},(= a\;\ti{t}{c})$.

        $C \vdash v : \epsilon;l;\phi_1^j,\phi_v,\phi_2^k \rightarrow \ti{t}{a};l;\phi_1^j,\phi_v,\phi_2^k,\ti{t}{a},(= a\;\ti{t}{c})$ by \refrule{Const}.

        \todo{Figure out if typing $v$ with a stale index variable is a problem.}

        $\phi_v \iff \phi_v,\ti{t}{a},(= a\;\ti{t}{c})$ trivially.

        $C \vdash v : \epsilon;l;\phi_1^j,\phi_v,\phi_2^k \rightarrow \ti{t}{a};l;\phi$ by \refrule{Implies}.
    ...
    \\ $\land$ $v_1^j \; v \; v_2^k;v' \; \<setlocal> j \hookrightarrow v_1^j \; v' \; v_2^k;\epsilon$
        ...
        By \reflemma{Inversion} on \refrule{Admin-Code}, $\vdash v : \ti{t}{a};\phi_v$,\\
        $S;C \vdash v' \; (\<setlocal> j) : \epsilon:l_1;\phi_1^j,\phi_v,\phi_2^k \rightarrow ti^{*};l;\phi$,\\
        $l_1(j) = \ti{t}{a}$, and $C_\text{local}(j) = t$.

        By \reflemma{Inversion} on \refrule{Admin-Const}, $t.\<const> c = v$, and\\
    ...
    \\ $\land$ $s;\<getglobal> j \hookrightarrow_i s_\text{glob}(i,j)$
        ...
        By \reflemma{Inversion} on \refrule{Program}, $\vdash s : S$, and $S;\epsilon \vdash_i v^{*};\<getglobal> j : ti^{*};l;\phi$.

        By \reflemma{Inversion} on \refrule{Code}, $S;C \vdash \<getglobal> j : \epsilon;l_1;\phi_v^{*} \rightarrow ti^{*};l;\phi$.

        By \reflemma{Inversion} on \refrule{Get-Global}, $ti^{*} = \ti{t}{a}$, $l = l_1$, $C_\text{global}(j) = \text{mut}^{?} t$,
        and $\phi_v^{*},\ti{t}{a} \implies \phi$.

        By \reflemma{Inversion} on \refrule{Store}, $S \vdash s_\text{inst}(i) : C$.

        Recalling that $C_\text{global}(j) = \text{mut}^{?} t$;
        by \reflemma{Inversion} on \refrule{Instance}, $\vdash s_\text{glob}(i,j) : \ti{t}{a_1};\phi_1$.
        ...
        $S;\epsilon \vdash_i v^{*};s_\text{glob}(i,j) : \ti{t}{a};l;\phi$ by \refrule{Code}, using the premises from \reflemma{Inversion}.
    ...
    \\ $\land$ $s;v \; (\<setglobal> j) \hookrightarrow_i s';\epsilon$, where $s' = s$ with $\text{glob}(i,j) = v$
        ...
        By \reflemma{Inversion} on \refrule{Program}, $\vdash s : S$, $S;\epsilon \vdash_i v_l^{*};v \; (\<setglobal> j) : ti^{*};l;\phi$.

        By \reflemma{Inversion} on \refrule{Code}, $S;C \vdash v \; (\<setglobal> j) : \epsilon;l_1;\phi_1 \rightarrow ti^{*};l;\phi$.
        ...
        By \reflemma{Inversion} on \refrule{Instance}, $\vdash s_\text{glob}(i,j) : \ti{t}{a_g};\phi_g$.
    ...
    \\ $\land$ $s;(\<ithreetwo>.\<const> k) \; (t.\<load> tp\_sx\; align\; o) \hookrightarrow_i s;(t.\<const> const_t(b^{*}))$, where $s_\text{mem}(i,k+o,|t|)=b^{*}$
        ...
        By \reflemma{Inversion} on \refrule{Composition}, \refrule{Const}, and \refrule{Load}, we know that $l_2=l_1$, $ti_2^{*} = ti_1^{*}\;\ti{t}{a}$ where $a \not\in \phi_3$, and $\satisfies{\phi_1,\ti{t}{a_1},(= a_1 \; \ti{\<ithreetwo>}{k})}{ti_1^{*}\;\ti{\<ithreetwo>}{a_1}\;l_1}{\phi_3}$.
        %%We also then know that $\satisfies{\phi_3,\ti{t}{a}}{ti_1^{*}\;\ti{\<ithreetwo>}{a}\;l_1}{\phi_2}$ by \reflemma{Inversion} on \refrule{Load}.

        \todo{I think this is only useful for store}
        Let $b_2^{*} = s_\text{mem}(i)$.
        We know that $C_\text{memory}=m$ because it is a premise of \refrule{Load} which we have assumed to hold by \reflemma{Inversion}.
