...
\begin{itemize}
    ...
    \\ $\land$ $\<local>_n \{ i;v^{*}_l \} \; v^n \<end> \hookrightarrow_j \; v^n$
        ...
        By $inversion$ on $const$, $\phi_l^{*},\phi_v^n \implies \phi_3$.

        Since $l_2$ are free in the reduced program, $\phi_v^n \implies \phi_3$.

        $S;C \vdash v^n : \epsilon;l_1;\phi_1 \rightarrow ti^n;l_2;\phi_1,\phi_v^n$ by $const$.
