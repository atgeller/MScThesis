...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \\ where $s;v^{*};e^{*} \hookrightarrow_i s';v'^{*};e'^{*}$
            ...
            We want to show that $\vdash_i s';v'^{*};L^k[e'^{*}] : ti^{*};l;\phi$.
            ...
            \begin{itemize}
                ...
                \item Case: $k > 0$
                    ...
                    Now, we invoke the \thought{I'm not sure I'm even doing induction right} inductive hypothesis and rebuild the type using the reduced expression.

                    Since $S;(ti^{*};l;\phi)^{*} \vdash_i v^{*};L^{k-1}[e^{*}] : ti_3^{*};l_2;\phi_2$, $s \vdash S$, and $s;v^{*};e^{*} \hookrightarrow_i s';v';e'^{*}$, then by the inductive hypothesis we know that $\vdash s' : S$ and $S;(ti^{*};l;\phi) \vdash_i v'^{*};L^{k-1}[e'^{*}] : ti_3^{*};l_2;\phi_2$.
